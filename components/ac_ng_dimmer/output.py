...
ac_ng_dimmer_ns = cg.esphome_ns.namespace("ac_ng_dimmer")
AcNgDimmer = ac_ng_dimmer_ns.class_("AcNgDimmer", output.FloatOutput, cg.Component)

DimMethod = ac_ng_dimmer_ns.enum("DimMethod")
DIM_METHODS = {
...
CONFIG_SCHEMA = cv.All(
    output.FLOAT_OUTPUT_SCHEMA.extend(
        {
            cv.Required(CONF_ID): cv.declare_id(AcNgDimmer),
...
async def to_code(config):
    if CORE.is_esp8266:
        # ac_ng_dimmer uses setTimer1Callback which requires the waveform generator
