...
static const char *const TAG = "ac_ng_dimmer";

// Global array to store dimmer objects
static AcNgDimmerDataStore *all_dimmers[32];  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
...
uint32_t IRAM_ATTR HOT AcNgDimmerDataStore::timer_intr(uint32_t now) {
...
void IRAM_ATTR HOT AcNgDimmerDataStore::gpio_intr() {
...
void IRAM_ATTR HOT AcNgDimmerDataStore::s_gpio_intr(AcNgDimmerDataStore *store) {
...
void IRAM_ATTR HOT AcNgDimmerDataStore::s_timer_intr() { timer_interrupt(); }
#endif

void AcNgDimmer::setup() {
  ...
  if (setup_zero_cross_pin) {
    ...
    this->zero_cross_pin_->attach_interrupt(&AcNgDimmerDataStore::s_gpio_intr, &this->store_,
                                            this->zero_cross_interrupt_type_);
  }

#ifdef USE_ESP8266
  // Uses ESP8266 waveform (soft PWM) class
  // PWM and AcNgDimmer can even run at the same time this way
  ...
  if (dimmer_timer == nullptr) {
    ...
    timer_attach_interrupt(dimmer_timer, &AcNgDimmerDataStore::s_timer_intr);
...
void AcNgDimmer::write_state(float state) {
